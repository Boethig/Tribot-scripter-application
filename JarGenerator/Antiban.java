...
public final class Antiban {
    ...
    public static void randomBankAfk() {
    	...
    	if (a == 1) {
    		debug("Leaving screen, afking 1-3 seconds");
    		leaveGame();
    		General.sleep(1000, 2500);
    	}
    	if (a == 10) {
    		debug("Leaving screen, afking 5-17 seconds");
    		leaveGame();
    		General.sleep(5142, 17231);

    	}
    	if (a == 17) {
    		debug("rotating camera and sleeping 3.5-6.7 seconds");
    		moveCamera();
    		General.sleep(3500, 6700);
    	}
    }
    ...
    public static void RunningActions(){
    	int a = General.random(1, 30);
    	if(a == 6) {
    		moveCamera();
    		debug("Camera rotated while running");
    	}
    	else if(a == 17) {
    		examineEntity();
    		debug("Entity examined while running");
    	}
    	else if(a == 23){
    		mouseMovement();
    		debug("Mouse moved while running");
    	}
    	else if(a == 28) {
    		rightClick();
    		debug("Right Clicked while running");
    	}
    }
