package scripts.JarGenerator.antiban;
...
public final class Antiban {
    ...
    static {
        abc = new ABCUtil();
        abc.setProperties(new ABCProperties());
        print_debug = false;
        resources_won = 0;
        resources_lost = 0;
    ...
    public static boolean switchTab(TABS TAB) {
 		switch (abc.generateTabSwitchPreference()) {
 		case F_KEY:
    ...
    public static void mouseWheelScroll() {
    	if (getABCUtil().shouldRotateCamera() || getABCUtil().shouldMoveMouse()) {
    		if (Options.isMouseScrollZoomEnabled()) {
    			int scroll = General.random(0, 3);
    			if (scroll == 0) {
    				Mouse.scroll(true, General.random(0, 3));
    			}
    			else if (scroll == 1) {
    				Mouse.scroll(false, General.random(0, 3));
    			}
    			else if (scroll == 2) {
    				Mouse.scroll(true, General.random(0, 2));
    				Mouse.scroll(false, General.random(0, 2));
    			}
    			else {
    				Mouse.scroll(false, General.random(0, 2));
    				Mouse.scroll(true, General.random(0, 2));
 ...
 /*    
    ...
    /**
	 * Resets the world hop tick
	 */
	public void resetWorldHop() {
		hopWorlds = false;
		nextWorld = -1;
	}

	/**
	 * Generates a world hop
	 */
	public void generateWorldHop() {
		hopWorlds = true;
	...
	public boolean shouldHopWorlds() {
		return new Boolean(hopWorlds) && new Integer(nextWorld) != -1;
	}
